...
def answer_question(
    ...
    history = history or []
    # NOTE: previously this only checked GROQ_API_KEY while .env.example/docs
    # referenced GROK_API_KEY — that mismatch meant the AI assistant silently
    # fell back to the template answer even when a key was configured. Both
    # names are now accepted so an existing Render env var keeps working.
