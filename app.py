...
# ---------------------------------------------------------------------------
# Role-based authorization, document ownership, audit log, notifications.
# Additive-only migrations — never drop/rewrite existing tables/data.
# ---------------------------------------------------------------------------
ALTER_USERS_SQL = [
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'viewer';",
]

# Safe one-time promotion: if NO account currently has the 'admin' role,
# promote the single oldest existing account (lowest id) to admin so the
# platform always has at least one administrator after this migration runs.
# This never touches an account if an admin already exists, and never lets
# a client request choose who becomes admin.
PROMOTE_FIRST_ADMIN_SQL = """
UPDATE users SET role = 'admin'
WHERE id = (SELECT MIN(id) FROM users)
  AND NOT EXISTS (SELECT 1 FROM users WHERE role = 'admin');
"""

ALTER_DOCUMENTS_OWNERSHIP_SQL = [
    "ALTER TABLE documents ADD COLUMN IF NOT EXISTS uploaded_by_user_id INTEGER REFERENCES users(id);",
]

ALTER_REPORTS_OWNERSHIP_SQL = [
    "ALTER TABLE reports ADD COLUMN IF NOT EXISTS generated_by_user_id INTEGER REFERENCES users(id);",
]
...
def init_db():
    ...
    try:
        with engine.begin() as conn:
            ...
            for stmt in ALTER_USERS_SQL:
                conn.execute(text(stmt))
            conn.execute(text(PROMOTE_FIRST_ADMIN_SQL))
            for stmt in ALTER_DOCUMENTS_OWNERSHIP_SQL:
                conn.execute(text(stmt))
            for stmt in ALTER_REPORTS_OWNERSHIP_SQL:
                conn.execute(text(stmt))
            conn.execute(text(CREATE_AUDIT_LOG_TABLE_SQL))
...
# Security / deployment: this app can serve its own frontend from the same
# origin (see serve_index() below), or be deployed split — frontend on
# Netlify, backend on Render, as this project currently is. For the split
# deployment, set CORS_ALLOWED_ORIGINS to a comma-separated list of exact
# trusted origins (e.g. "https://dancing-caramel-606aba.netlify.app") —
# never a wildcard "*", since that is incompatible with allow_credentials.
# The session cookie is set with SameSite=None; Secure=True (see
# _set_session_cookie below), which is what cross-origin fetch() with
# credentials:"include" requires; the browser will refuse to send it
# otherwise. Secure=True requires the request to arrive over HTTPS, which
# Render provides — confirmed via the X-Forwarded-Proto header set by
# Render's proxy.
...
def _get_session_user(conn, token: str | None) -> dict | None:
    ...
    row = conn.execute(
        text(
            """
            SELECT s.id AS session_id, s.expires_at, u.id AS user_id, u.email, u.created_at, u.role
            FROM sessions s
    ...
    if auth.is_expired(row["expires_at"]):
        ...
        return None
    # Role always comes from the database row looked up server-side from the
    # session token — it is never accepted from the client/request.
    return {"id": row["user_id"], "email": row["email"], "created_at": row["created_at"], "role": row["role"]}
...
def require_admin(user: dict = Depends(require_auth)) -> dict:
    """
    Required-auth + required-role dependency. The role is always the value
    looked up server-side in require_auth (from the `users` table via the
    session token) — a request body/header claiming a different role has
    no effect. Returns HTTP 403 for any authenticated non-admin.
    """
...
def record_audit(
    ...
    """
    Best-effort audit log insert. Never raises — an audit-log failure must
    never break the primary request. Never stores passwords, session
    tokens, or other secrets; `details` should only ever contain
    human-readable, non-sensitive summary text.
    """
    if engine is None:
        return
    try:
        conn.execute(
            text(
                """
                INSERT INTO audit_log (user_id, action, document_id, report_id, details)
                VALUES (:user_id, :action, :document_id, :report_id, :details)
                """
            ),
            {
                "user_id": user_id,
...
def create_notification(conn, user_id: int | None, message: str, ntype: str = "info") -> None:
    """Best-effort notification insert — never fabricated, only called for real events."""
    if engine is None or user_id is None:
        return
    try:
        conn.execute(
            text("INSERT INTO notifications (user_id, message, type) VALUES (:user_id, :message, :type)"),
            {"user_id": user_id, "message": message[:500], "type": ntype},
        )
    except OperationalError:
        pass
...
def signup(payload: SignupRequest, request: Request, response: Response):
    ...
    try:
        with engine.begin() as conn:
            ...
            # New accounts always start as 'viewer' — a signup request can never
            # set its own role (PROMOTE_FIRST_ADMIN_SQL is the only migration-time
            # exception, and only applies once, if no admin exists at all).
            user_row = conn.execute(
                text(
                    "INSERT INTO users (email, password_hash, role) VALUES (:email, :hash, 'viewer') "
            ...
            record_audit(conn, user_row["id"], "ACCOUNT_CREATED", details=f"New account: {email}")
...
def login(payload: LoginRequest, request: Request, response: Response):
    ...
    try:
        with engine.begin() as conn:
            ...
            record_audit(conn, user_row["id"], "LOGIN", details=f"{email} signed in")
        _set_session_cookie(response, request, token)
        return {"id": user_row["id"], "email": user_row["email"], "role": user_row["role"], "created_at": user_row["created_at"]}
    except HTTPException:
        raise
    except OperationalError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def logout(response: Response, session_token: str | None = Cookie(default=None)):
    if engine is not None and session_token:
        try:
            with engine.begin() as conn:
                who = conn.execute(
                    text("SELECT user_id FROM sessions WHERE token = :token"), {"token": session_token}
                ).scalar()
                conn.execute(text("DELETE FROM sessions WHERE token = :token"), {"token": session_token})
                if who is not None:
                    record_audit(conn, who, "LOGOUT")
        except OperationalError:
...
def health():
    ...
    result = {
        ...
        "database": "unknown",
        "database_error": None,
        # Booleans only — never expose the actual key value.
        "ai_configured": bool(os.getenv("GROK_API_KEY") or os.getenv("GROQ_API_KEY")),
        "storage_writable": None,
    }

    # Storage check: confirm the upload/report directories actually exist
    # and are writable, rather than assuming so just because the process
    # started successfully.
    try:
        probe_path = os.path.join(UPLOAD_DIR, ".health_probe")
    ...
    try:
        with engine.begin() as conn:
            row = conn.execute(
                text(
                    """
                    INSERT INTO documents (filename, file_type, status, file_path, uploaded_by_user_id)
                    VALUES (:filename, :file_type, 'uploaded', :file_path, :uploaded_by)
                    RETURNING id, filename, file_type, status, sheet_name, total_rows,
                              extracted_rows, error_message, uploaded_at, processed_at, uploaded_by_user_id
                    """
                ),
                {
            ...
            record_audit(conn, user["id"], "DOCUMENT_UPLOADED", document_id=row["id"], details=original_filename)
        result = dict(row)
        result["uploaded_by_email"] = user["email"]
        return result
    except OperationalError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def extract_document_endpoint(document_id: int, user: dict = Depends(require_admin)):
    """
    ...
    try:
        with engine.begin() as conn:
            ...
            updated = conn.execute(
                text(
                    """
                    SELECT d.id, d.filename, d.file_type, d.status, d.sheet_name, d.total_rows,
                           d.extracted_rows, d.error_message, d.uploaded_at, d.processed_at,
                           d.uploaded_by_user_id, u.email AS uploaded_by_email
            ...
            owner_id = updated["uploaded_by_user_id"]
            if updated["status"] == "extracted":
                record_audit(conn, user["id"], "DOCUMENT_EXTRACTED", document_id=document_id,
                              details=f"{updated['extracted_rows']} row(s) extracted from '{updated['filename']}'")
                create_notification(conn, owner_id, f"Document processing completed: {updated['filename']}", "success")
            else:
                record_audit(conn, user["id"], "DOCUMENT_EXTRACTED", document_id=document_id,
                              details=f"Extraction ended with status '{updated['status']}' for '{updated['filename']}'")
                create_notification(conn, owner_id, f"Document processing failed: {updated['filename']}", "error")
        return dict(updated)
    except OperationalError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def seed_sample(user: dict = Depends(require_admin)):
    """
...
def generate_report(document_id: int, user: dict = Depends(require_admin)):
    """
    ...
    try:
        with engine.begin() as conn:
            report_id = conn.execute(
                text(
                    """
                    INSERT INTO reports (document_id, title, dataset_period, status, generated_by_user_id)
                    VALUES (:document_id, :title, :dataset_period, 'generating', :generated_by)
                    RETURNING id
                    """
                ),
                {
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            record_audit(conn, user["id"], "REPORT_GENERATED", document_id=document_id, report_id=report_id,
                         details=row["title"])
            create_notification(conn, user["id"], f"Report generation completed: {row['title']}", "success")
        result = dict(row)
        ...
        try:
            with engine.begin() as conn:
                ...
                create_notification(conn, user["id"], f"Report generation failed for document #{document_id}.", "error")
        except OperationalError:
            pass
...
def assistant_ask(payload: AssistantAskRequest, user: dict = Depends(require_auth)):
    ...
    try:
        with engine.begin() as conn:
            record_audit(conn, user["id"], "AI_QUERY", document_id=payload.document_id, details=question[:300])
    except OperationalError:
        pass
...
async def assistant_ask_image(
    ...
    try:
        ...
        with engine.begin() as conn:
            record_audit(conn, user["id"], "AI_QUERY", document_id=document_id,
                         details=f"(image) {question[:250]}" if question else "(image only)")
    except OperationalError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
...
# ---------------------------------------------------------------------------
# Step 9 — Audit history endpoints
# Viewer: own activity only. Admin: platform-wide, with filters/pagination.
# ---------------------------------------------------------------------------
...
@app.get("/api/history", response_model=AuditPage)
def my_history(limit: int = 50, offset: int = 0, user: dict = Depends(require_auth)):
    """Returns only the authenticated user's own activity — never another user's."""
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    limit = max(1, min(limit, 200))
    offset = max(0, offset)
    try:
        with engine.connect() as conn:
            total = conn.execute(
                text("SELECT COUNT(*) FROM audit_log WHERE user_id = :uid"), {"uid": user["id"]}
            ).scalar()
            rows = conn.execute(
                text(
                    """
                    SELECT a.id, a.user_id, u.email AS user_email, a.action, a.document_id,
...
def admin_activity(
    ...
    """Platform-wide activity feed. Admin-only — HTTP 403 for any non-admin caller."""
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    limit = max(1, min(limit, 200))
    offset = max(0, offset)
    ...
    if document_id is not None:
        where.append("a.document_id = :document_id")
        params["document_id"] = document_id
    where_sql = f"WHERE {' AND '.join(where)}" if where else ""
    try:
        with engine.connect() as conn:
            total = conn.execute(
                text(f"SELECT COUNT(*) FROM audit_log a LEFT JOIN users u ON u.id = a.user_id {where_sql}"), params
            ).scalar()
            rows = conn.execute(
                text(
                    f"""
...
def list_notifications(limit: int = 30, user: dict = Depends(require_auth)):
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
...
def mark_notification_read(notification_id: int, user: dict = Depends(require_auth)):
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    try:
        with engine.begin() as conn:
            # Ownership check — a user may only mark their own notification read.
            row = conn.execute(
                text(
                    """
...
def mark_all_notifications_read(user: dict = Depends(require_auth)):
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    try:
        with engine.begin() as conn:
...
# ---------------------------------------------------------------------------
# Step 9 — Word Cloud & Topic Intelligence
#
# Built ONLY from real, already-extracted data: the `evidence.raw_value`
# and `mines.mine_name` columns captured during extraction (Step 2/3).
# XLSX extraction does not currently retain free-form document prose, so
# there is no separate "document text" corpus to mine — this is the actual
# extracted text content that exists in the database today. Nothing here
# is fabricated; if there is no extracted data yet, the response says so.
# ---------------------------------------------------------------------------
_STOPWORDS = {
    "the","a","an","of","and","or","to","in","on","for","with","by","is","are",
    "was","were","this","that","it","as","at","from","be","not","no","na","nil",
    "none","unknown","other","others","total","mine","mines","colliery","project",
    "pvt","ltd","limited","company","co","area","block","dist","district","state",
}


def _tokenize(value: str) -> list[str]:
    return [w for w in re.findall(r"[A-Za-z]{3,}", value or "") if w.lower() not in _STOPWORDS]


@app.get("/api/analytics/wordcloud")
def get_wordcloud(document_id: int | None = None, limit: int = 60, user: dict = Depends(require_auth)):
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    where_sql = "WHERE document_id = :document_id" if document_id is not None else ""
    params = {"document_id": document_id} if document_id is not None else {}
    try:
        with engine.connect() as conn:
            ev_rows = conn.execute(
                text(f"SELECT raw_value FROM evidence {where_sql}"), params
            ).all()
            name_rows = conn.execute(
                text(f"SELECT mine_name, state_ut, district, owner_full FROM mines {where_sql}"), params
            ).all()
    except OperationalError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    ...
    counts: dict[str, int] = {}
    for (raw,) in ev_rows:
        for w in _tokenize(str(raw) if raw is not None else ""):
            counts[w] = counts.get(w, 0) + 1
    for mine_name, state_ut, district, owner_full in name_rows:
        for field in (mine_name, state_ut, district, owner_full):
            for w in _tokenize(field or ""):
                counts[w] = counts.get(w, 0) + 1

    if not counts:
        return {"terms": [], "source": "extracted_data", "message": "No processed document text is available yet."}

    top = sorted(counts.items(), key=lambda kv: kv[1], reverse=True)[:max(1, min(limit, 200))]
    return {
        "terms": [{"term": t, "count": c} for t, c in top],
...
def get_topics(document_id: int | None = None, user: dict = Depends(require_auth)):
    """
    Deterministic, keyword-based topic grouping over real extracted field
    values (never a second unrelated AI provider — Grok, already used by
    the AI Assistant/report narrative, is the only LLM integration in this
    project, and topic labels here are computed without calling it, so the
    result is reproducible and clearly not AI-generated).
    """
    if engine is None:
        raise HTTPException(status_code=503, detail="Database not configured.")
    where_sql = "WHERE document_id = :document_id" if document_id is not None else ""
    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                text(
                    f"""
                    SELECT mine_name, state_ut, district, owner_full, coal_or_lignite, mine_type,
                           ownership_label, validation_status
                    FROM mines {where_sql}
                    """
                ),
                params,
            ).all()
    ...
    if not rows:
        return {"topics": [], "source": "extracted_data", "message": "No processed document data is available yet."}

    topic_defs = {
        "Production": lambda r: True,  # every mine record is a production record
        "Geology": lambda r: bool(r[4]),  # coal_or_lignite present
        "Mining Operations": lambda r: bool(r[5]),  # mine_type present
        "Government Reporting / Ownership": lambda r: bool(r[6]),  # ownership_label present
        "Location & Geography": lambda r: bool(r[1] or r[2]),  # state/district present
        "Data Validation": lambda r: bool(r[7]),
    }
    documents_seen = None
    try:
        with engine.connect() as conn:
            documents_seen = conn.execute(
                text(f"SELECT COUNT(DISTINCT document_id) FROM mines {where_sql}"), params
            ).scalar()
    except OperationalError:
        pass

    topics = []
    for label, predicate in topic_defs.items():
        matching = [r for r in rows if predicate(r)]
        if not matching:
            continue
        keywords = sorted({w for r in matching for f in (r[0], r[1], r[2], r[3]) for w in _tokenize(f or "")})[:8]
        topics.append({
            "topic": label,
    ...
    return {"topics": topics, "source": "extracted_data (deterministic keyword grouping)", "message": None}
